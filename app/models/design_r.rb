class DesignR < ApplicationRecord
  ...
  def calcExudateScore
    ...
    exudateCode = extractCode(exudate)
    score =
      case exudateCode
      when 'e0' then 0
      when 'e1' then 1
  ...
  def calcInflammationScore
    ...
    inflammationCode = extractCode(inflammation)
    score =
  ...
  def calcGranuleTissueScore
    ...
    granule_tissueCode = extractCode(granule_tissue)
    score =
  ...
  def calcNecroticTissueScore
    ...
    necrotic_tissueCode = extractCode(necrotic_tissue)
    score =
